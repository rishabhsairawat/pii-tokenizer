...
RSpec.describe PiiTokenizer::EncryptionService do
  ...
  describe '#encrypt_batch' do
    ...
    it 'correctly generates different keys based on input data' do
      ...
      response_body = {
        data: [
          { entity_type: 'customer', entity_id: '123', pii_type: 'NAME', token: 'customer_token' },
          { entity_type: 'employee', entity_id: '456', pii_type: 'NAME', token: 'employee_token' }
        ]
      }.to_json
      ...
      result = service.encrypt_batch(tokens_data)
      expect(result).to eq(
        'CUSTOMER:123:NAME' => 'customer_token',
        'EMPLOYEE:456:NAME' => 'employee_token'
      )
    end
